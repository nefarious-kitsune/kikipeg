...
function generateJavascript(ast, options) {
  ...
  var generate = visitor.build({
    ...
    sequence: function(node, context) {
      if (node.elements.length === 1) {
        ...
        const allParts = []; // Array containing all parts
        const pickedParts = []; // Array containing only the picked parts
        let i;
        ...
        for (i = 0; i < node.elements.length; i++) {
          ...
          subresult.free = [];
          result.append(subresult);
          allParts.push(subresult.expression);
          if (subresult.picked) pickedParts.push(subresult.expression);
        ...
        if (context.getDiscard()) {
          ...
          if (pickedParts.length === 0) {
            result.block.push(`${resultReg} = [${allParts.join(',')}];`);
          } else if (pickedParts.length === 1) {
            result.block.push(`${resultReg} = ${pickedParts[0]};`);
          } else {
            result.block.push(`${resultReg} = [${pickedParts.join(',')}];`);
          }
        }
        result.block.push(language.blockEnd(label));
    ...
    picked: function(node, context) {
      ...
      const newContext = context.cloneEnv();
      result.append(recurse(node.expression, newContext));
      result.picked = true;
      return result;
    },
  ...
  if (options.headerComment) {
    ...
    headerComment = '/*\n' +
                    ' * Generated by KikiPEG\n' +
                    ' */\n\n';
  }
  templateVars['/*HEADER_COMMENT*/'] = headerComment;

  let mainCode = fs.readFileSync(__dirname + '/../../runtime/template.js', 'utf8');

  for (const [name, value] of Object.entries(templateVars)) {
    const matcher =
        new RegExp(`(^ *)?${name.replaceAll('*', '\\*')}`, 'mg');
    const replacer =
        (m, padding) => padding?value.replace(/^/mg, padding):value;
    mainCode = mainCode.replace(matcher, replacer);
  }
  ...
  if (options.format == 'esm') {
    ...
    if (options.dependencies) {
      for (const [imported, importSource] of Object.entries(options.dependencies)) {
        headerCode += `import ${imported} from '${importSource}';\n`;
      }
    }
    footerCode =
      'export {\n' +
    ...
    if (options.dependencies) {
      for (const [imported, importSource] of Object.entries(options.dependencies)) {
        headerCode += `var ${imported} = require('${importSource}');\n`;
      }
    }
    footerCode =
      'module.exports = {\n' +
