...
function generateJavascript(ast, options) {
  ...
  Context.prototype = {
     ...
     * some sub-expressions will use this information to return true instead of
     * the match result.
     */
    ...
    sequence: function(node, context) {
      if (node.elements.length === 1) {
        ...
        for (i = 0; i < node.elements.length; i++) {
          ...
          // since failing sub-expressions do not increment the position.
          if (i > 0) {
            result.onFailure([`${language.currPos} = ${posReg};`]);
          }
  ...
  const template =
      fs.readFileSync(__dirname + '/../../runtime/template.js', 'utf8');
